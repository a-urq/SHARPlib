...
struct lifter_peters_et_al {
    ...
    float* pressure_arr;
    float* height_arr;
    float* temperature_arr;
    float* virtemp_arr;
    float* dewpoint_arr;
    float* u_wind_arr;
    float* v_wind_arr;
    std::ptrdiff_t prof_N;
    ...
    [[nodiscard]] inline float operator()(float pres, float tmpk,
        ...
        if(can_do_efficient_lift) {
            pcl_tmpk = moist_adiabat_peters_et_al(pressure, temperature, 
                new_pres,  this->qv, this->qt, pressure_arr, height_arr, 
     ...
     */
    void set_profile(const float pressure[], const float height[],
            const float temperature[], const float dewpoint[],
            const float u_wind[], const float v_wind[], const std::ptrdiff_t N) {
        pressure_arr = new float[N];
        std::copy(pressure, pressure + N, pressure_arr);

        height_arr = new float[N];
        std::copy(height, height + N, height_arr);
        
        temperature_arr = new float[N];
        std::copy(temperature, temperature + N, temperature_arr);
        
        dewpoint_arr = new float[N];
        std::copy(dewpoint, dewpoint + N, dewpoint_arr);
        
        u_wind_arr = new float[N];
        std::copy(u_wind, u_wind + N, u_wind_arr);
        
        v_wind_arr = new float[N];
        std::copy(v_wind, v_wind + N, v_wind_arr);

        prof_N = N;

        virtemp_arr = new float[N];
        for(int i = 0; i < N; i++) {
            float rv = sharp::saturation_mixing_ratio(pressure[i], dewpoint_arr[i], 1);
            float qv = sharp::specific_humidity(dewpoint_arr[i]);
            virtemp_arr[i] = sharp::density_temperature(temperature_arr[i], qv, qv);
        }
    }
     ...
     */
    void determine_entrainment_rate(Parcel pcl,
        ...
        float* pcl_vtmpk_arr = pressure_arr = new float[prof_N];
        pcl.lift_parcel(lifter, pressure_arr, pcl_vtmpk_arr, prof_N);

        float* buoy_arr = pressure_arr = new float[prof_N];
        buoyancy(pcl_vtmpk_arr, virtemp_arr, buoy_arr, prof_N);
        pcl.cape_cinh(pressure_arr, height_arr, buoy_arr, prof_N);

        entr_rate = compute_epsilon(pressure_arr, height_arr, 
            temperature_arr,
            dewpoint_arr, u_wind_arr, v_wind_arr, prof_N, 
     ...
     * \author Amelia Urquhart - OU-SoM
     *
     * \brief Helper for entrainment rate determination.
     *
     * Mostly copied from Kelton Halbert's implementation of the ECAPE value
     * function, but adjusted to determine the entrainment rate rather than
     * ECAPE itself.
     *
    ...
    float compute_epsilon(const float pressure[], const float height[],
                        const float temperature[], const float dewpoint_arr[],
        ...
        float *mse_arr = new float[N];
        const float hsfc = height[0];
        for (int k = 0; k < N; ++k) {
            const float dwpk = dewpoint_arr[k];
            const float rv = mixratio(pressure[k], dwpk);
            const float qv = specific_humidity(rv);
            const float height_agl = height[k] - hsfc;
            const float mse_star = moist_static_energy(height_agl, dwpk, qv);
        }
